...
def lock_every_seed(seed_value=42):
    """Pin every RNG source so repeated runs stay deterministic."""
...
def locate_dataset_root():
    """Return the dataset root path depending on the execution environment."""
    import os
    if os.path.exists("/kaggle/input"):
        return pathlib.Path("/kaggle/input/datasets/huynhthethien/radarcommunsignaldata2026train")
...
class ResidualMobileUnit(nn.Module):
    ...
    stride=1 and matching channel counts -> identity skip path.
    """
...
# Aliases – do not change model architecture
...
def show_parameter_budget(network):
    ...
    assert trainable < 100_000, f"Parameter budget exceeded: {trainable:,} >= 100,000"
...
def run_training_pass(network, batch_stream, loss_fn, opt, sched, num_classes):
    network.train()
    cum_loss     = 0.0
...
def append_best_epoch_log(
    ...
    log_path = pathlib.Path(log_path) if log_path else LOGS_DIR / "best_epoch_log.txt"
...
def execute_training_loop(
    network, training_batches, validation_batches, loss_fn,
    optimizer, scheduler, history, num_classes, class_names
):
    """
    Full training loop with early stopping.
    ``class_names`` is passed explicitly (no globals() lookup).
    """
    best_val_acc       = 0.0
    stored_preds       = []
    stored_tgts         = []
...
def save_loss_chart(store):
    ...
    plt.savefig(PLOTS_DIR / "loss_plot.png", dpi=300); plt.close()
...
def save_accuracy_chart(store):
    ...
    plt.legend(fontsize=12); plt.grid(True, alpha=0.4); plt.tight_layout()
    plt.savefig(PLOTS_DIR / "accuracy_plot.png", dpi=300); plt.close()
...
def save_confusion_matrix_chart(targets, predictions, label_names):
    ...
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / "confusion_matrix.png", dpi=300); plt.close()
...
def main():
    ...
    final_labels, final_preds = execute_training_loop(
        ...
        criterion, optimizer, scheduler, history,
    ...
    # ------------------------------------------------------------------
    # Confusion matrix (computed on the full training set for speed)
    # ------------------------------------------------------------------
    train_targets = []
    train_preds   = []
    ...
    GroupID        = "03"
    model_name     = f"{GroupID}_DeepLearning_Project_TrainedModel.pt"
    traced_model.save(OUTPUT_DIR / model_name)
    print(f"[INFO] Model saved: {OUTPUT_DIR / model_name}")
